...
def standardize_column_names(
    df: DataFrame, naming_convention: str = "snake_case"
) -> DataFrame:
    """
    ...
    if naming_convention == "snake_case":
        ...
        for col_name in df.columns:
            ...
            new_name = (
                regexp_replace(lit(col_name), r"[^a-zA-Z0-9]", "_")
                .alias("temp")
                .collect()[0][0]
            )
...
def add_audit_columns(df: DataFrame, load_timestamp: Optional[str] = None) -> DataFrame:
    ...
    result_df = (
        df.withColumn("load_timestamp", timestamp_col)
        .withColumn("source_system", lit("pyspark_etl"))
        .withColumn("load_id", lit("batch_load"))
    )  # Could be replaced with actual batch ID
...
def create_date_dimensions(df: DataFrame, date_column: str) -> DataFrame:
    ...
    """
    result_df = (
        df.withColumn(f"{date_column}_year", year(col(date_column)))
        .withColumn(f"{date_column}_month", month(col(date_column)))
        .withColumn(f"{date_column}_day", dayofmonth(col(date_column)))
        .withColumn(
            f"{date_column}_formatted", date_format(col(date_column), "yyyy-MM-dd")
        )
    )
...
def apply_business_rules(df: DataFrame, rules: Dict[str, Any]) -> DataFrame:
    ...
    for rule_name, rule_config in rules.items():
        if rule_config["type"] == "filter":
            ...
            logger.info(
                "Applied column derivation rule", rule=rule_name, column=column_name
            )
            ...
            result_df = result_df.withColumn(
                f"{column_name}_category", when_expr.otherwise("Other")
            )
            logger.info(
                "Applied categorization rule", rule=rule_name, column=column_name
            )
...
def aggregate_metrics(
    ...
    for column, agg_func in metrics.items():
        alias_name = (
            f"{alias_prefix}{column}_{agg_func}"
            if alias_prefix
            else f"{column}_{agg_func}"
        )
