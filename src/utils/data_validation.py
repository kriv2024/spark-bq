...
from pyspark.sql.functions import col, count, isnan, isnull, when  # noqa: F401
...
def check_data_quality(
    ...
    total_rows = df.count()
    quality_report = {"total_rows": total_rows, "columns": {}}
    ...
    for column in columns:
        ...
        quality_report["columns"][column] = {
            ...
            "completeness": (
                ((total_rows - null_count) / total_rows) * 100 if total_rows > 0 else 0
            ),
        }
...
def remove_duplicates(
    df: DataFrame, subset: Optional[List[str]] = None, keep: str = "first"
) -> DataFrame:
    """
...
def handle_null_values(
    ...
    logger.info(
        "Null values handled", strategy=strategy, subset=subset or "all_columns"
    )
