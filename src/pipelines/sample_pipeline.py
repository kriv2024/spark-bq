...
from src.utils.data_validation import check_data_quality, validate_required_columns
...
class SamplePipeline:
    ...
    def extract_data(
        self, source_table: str, filter_condition: Optional[str] = None
    ) -> DataFrame:
        """
        ...
        df = self.bq_connector.read_table(
            table_id=source_table, filter_condition=filter_condition
        )
    ...
    def transform_data(self, df: DataFrame) -> DataFrame:
        ...
        logger.info(
            "Data quality check completed", total_rows=quality_report["total_rows"]
        )
        ...
        for date_col in date_columns[:1]:  # Process first date column found
            try:
                ...
                logger.warning(
                    "Failed to create date dimensions", column=date_col, error=str(e)
                )

        logger.info(
            "Data transformation completed", final_columns=len(transformed_df.columns)
        )
    ...
    def load_data(
        ...
        """
        logger.info("Starting data load", target_table=target_table, mode=mode)

        self.bq_connector.write_table(
            df=df, table_id=target_table, mode=mode, partition_by=partition_by
        )
    ...
    def run_pipeline(
        ...
        """
        try:
            logger.info("Pipeline started", source=source_table, target=target_table)
